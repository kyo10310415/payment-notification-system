...
| 項目 | 値 | 説明 |
|------|-----|------|
| **Schedule** | `0 0,12 * * *` | 毎日 AM9時とPM9時に実行（JST） |
...
│  │   │ └─── 月 (1-12)
│  │   └───── 日 (1-31)
│  └───────── 時 (0,12 = UTC 00:00と12:00)
└────────────── 分 (0 = 0分)
```

**実行タイミング（JST基準）:**
- **AM 9:00** (UTC 00:00) - 過去12時間のメッセージを監視
- **PM 9:00** (UTC 12:00) - 過去12時間のメッセージを監視

**注意:** Renderのタイムゾーンは **UTC** です。日本時間（JST = UTC+9）での実行時刻は以下の通りです：

| 日本時間（JST） | UTC時間 | Cron式 |
|---------------|---------|-------|
| 毎日 AM 9:00 | 毎日 00:00 | `0 0 * * *` |
| 毎日 PM 9:00 | 毎日 12:00 | `0 12 * * *` |
| 毎日 AM 9:00 & PM 9:00 | 毎日 00:00 & 12:00 | `0 0,12 * * *` |

**その他のスケジュール例:**
- `0 */1 * * *` - 1時間ごと
- `0 */6 * * *` - 6時間ごと
- `0 0 * * *` - 毎日 0:00 (UTC) = 日本時間 9:00
- `0 9,15,21 * * *` - 毎日 9:00, 15:00, 21:00 (UTC)
